...
conn.on('ready', () => {
    conn.sftp((err, sftp) => {
        const stream = sftp.createWriteStream('/tmp/ruijie_test_final.js');
        const script = `
        ...
        function fetchRuijieRaw(host, port, user, pass) {
            return new Promise((resolve, reject) => {
                ...
                client.on('data', (data) => {
                    ...
                    process.stdout.write(chunk); // Echo characters
                    output += chunk;
        ...
        stream.end();
        stream.on('close', () => {
            conn.exec("echo '!Tahun2026' | sudo -S docker cp /tmp/ruijie_test_final.js librenms_dashboard:/app/ruijie_test_final.js && echo '!Tahun2026' | sudo -S docker exec librenms_dashboard node /app/ruijie_test_final.js", (err, stream2) => {
                stream2.pipe(process.stdout);
                stream2.stderr.pipe(process.stderr);
