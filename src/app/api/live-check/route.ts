...
function getCommand(vendor: string, checkType: CheckType, peerIp: string): string {
    const cmds: Record<string, Record<CheckType, string>> = {
        ...
        vyos: {
            'bgp-status':        `vtysh -c "show ip bgp neighbors ${peerIp}"`,
            'received-routes':   `vtysh -c "show ip bgp neighbors ${peerIp} received-routes"`,
            'advertised-routes': `vtysh -c "show ip bgp neighbors ${peerIp} advertised-routes"`,
            'ping':              `/bin/ping -c 5 ${peerIp}`,
            'logs':              `cat /var/log/messages | grep ${peerIp} | tail -n 50`,
        },
        danos: {
            'bgp-status':        `/bin/vbash -ic "show protocols bgp ipv4 unicast neighbors ${peerIp}"`,
            'received-routes':   `/bin/vbash -ic "show protocols bgp ipv4 unicast neighbors ${peerIp} received-routes"`,
            'advertised-routes': `/bin/vbash -ic "show protocols bgp ipv4 unicast neighbors ${peerIp} advertised-routes"`,
            'ping':              `/bin/ping -c 5 ${peerIp}`,
            'logs':              `cat /var/log/messages | grep ${peerIp} | tail -n 50`,
        },
    };
